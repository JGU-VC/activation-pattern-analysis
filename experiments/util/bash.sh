...
function printTable()
{
    ...
    then
        ...
        then
            ...
            then
                local output=''
                output="$(echo -e "${table}" | column -s '#' -t | perl -lpe's/^\s*//g' | awk '/^\+/{gsub(" ", "─", $0)}1' )"

                if [[ "${sort}" = 'true' ]]; then
                    output_sorted="$(echo -e "$output" | head -n -1 | tail -n +4 | sort)"
                    output="$(echo -e "$output" | head -n 3)\n${output_sorted}
$(echo -e "$output" | tail -n 1)"
        ...
        then
            echo -e "\n\033[1;36mTOTAL $displayTotalCount : $((numberOfLines - 1))\033[0m"
        fi
    fi
}
