"""
Bottleneck building block for the PyramidNet architecture.
...
class BottleneckPyramid(nn.Module):

    def __init__(self, state, event, in_planes, planes, stride=1, downsample=None, dilation=1):
        del downsample
        self.event = event
        self.state = state
        ...
        self.block_seq = nn.Sequential(
            ...
            event.filter_layer(in_planes, planes, kernel_size=1, stride=1, bias=False),
            event.normalization_layer(planes),
            event.activation_layer(inplace=state["inplace"]),
            event.filter_layer(planes, planes, kernel_size=3, stride=stride, padding=dilation, groups=state["groups"], bias=False, dilation=dilation),
            event.normalization_layer(planes),
            event.activation_layer(inplace=state["inplace"]),
            event.filter_layer(planes, out_planes, kernel_size=1, stride=1, bias=False),
...
def register(mf):
    ...
    mf.register_defaults({
        ...
        "downsample": 'avgpool',
        "expansion": 4,  # original paper uses 4
        "groups": 1,
    ...
    mf.register_event("resblock", BottleneckPyramid, unique=True)
